...
forceaddSQL = "INSERT INTO VMS_volunteer_availability(person_pk, event_id, free_time_start) VALUES(%s, %s, %s);"
...
for dt in datetime_to_time_node(str(return_data[0][0]),str(return_data[0][1])):
	...
	try:
		...
		if cursor.rowcount < 1:
			cursor.execute(forceaddSQL,[data['volunteerId'],data['eventId'],dt])
		else:
